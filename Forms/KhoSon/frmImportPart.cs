...
namespace BMS
{
	public partial class frmImportPart : _Forms
	{
		...
		bool SaveData()
		{
			...
			if (MessageBox.Show(String.Format("Bạn có chắc muốn lưu không?"), TextUtils.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				if (txbQuantity.Value > 0)
				{
					...
					ArrayList arr = SONHistoryImExBO.Instance.FindByExpression(exp1.And(exp2));
					if (arr.Count > 0)
					{
					...
					//Check trong Kế hoạch sơn nếu có thì update số lượng và 
		...
		void LoadDataToChart()
		{
			...
			DataTable dt = TextUtils.LoadDataFromSP("spGetSonPartImportDataByDate", "A", new string[] { "@StartDate", "@EndDate" }, new object[] { startDate, endDate });
		...
		private void txbPartCode_KeyPress(object sender, KeyPressEventArgs e)
		{
			if (e.KeyChar == (char)13)
			{
				try
				{
					int Quantity = TextUtils.ToInt(TextUtils.ExcuteScalar($"SELECT Quantity FROM[ShiStock].[dbo].[SONHistoryImEx] WHERE OrderCode =N'{txbOrderCode.Text.Trim()}' AND PartCode = N'{txbPartCode.Text.Trim()}'"));
